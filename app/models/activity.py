...
/vouchers/mine (который показывает вексели, которые вы держите СЕЙЧАС),
сюда попадают события обеих ролей одного и того же пользователя:

  VOUCHER_MINTED    — вы выпустили (заминтили) новый вексель из своих УЕ
                      в реестре — до этого момента продать его было нельзя
  LISTING_CREATED   — вы выставили СВОЙ вексель на продажу (продавец)
  LISTING_CANCELLED — вы сняли объявление с продажи (продавец)
  SALE              — у вас купили вексель по вашему объявлению (продавец)
  SALE_CANCELLED    — покупатель отменил покупку вашего векселя (продавец)
  PURCHASE          — вы купили вексель(я) на рынке (покупатель)
  VOUCHER_REDEEMED  — вы обналичили вексель, УЕ зачислены на баланс (покупатель)
  VOUCHER_CANCELLED — вы отменили свою покупку векселя (покупатель)
